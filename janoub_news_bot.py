...
قواعد صياغة الملخص التشويقي (Smart Hook) الخاصة بحقل (excerpt):
...
• أعد النتيجة في كائن JSON يحتوي على الحقول التالية:
  ...
  - excerpt: الملخص التشويقي الذكي (Smart Hook) بحدود 220 حرفاً وفق التعليمات المحددة أعلاه.
  - content: نص الخبر الكامل والموسع أعيد كتابته دون اختصار.
  - houthi_iran_exclude: قيمة boolean (true للخبر المحظور، و false للخبر المقبول).
"""
...
"""
    ...
    # 2. تعليمات صياغة الملخص التشويقي (Smart Hook) بشكل مستقل
    excerpt_instruction = """
### قواعد صياغة حقل الملخص (excerpt) - التشويق الذكي (Smart Hook):
- صغ حقل (excerpt) كـ "عنوان إخباري تشويقي ومكثف" في فقرة واحدة متدفقة ومترابطة (لا تتجاوز 220 حرفاً).
- اعرض زبدة الخبر والقرار/الحدث الرئيس بروح الوكالات العالمية مع ربطه بالسبب الأمني أو السياسي المباشر.
- طَبّق قاعدة التشويق الذكي (Smart Hook): اترك نقطة غموض سياسية أو تداعيات غير مكتملة في نهاية جملة الملخص تدفع القارئ فوراً لقراءة باقي التفاصيل داخل المتن (content).
- يُمنع تماماً استخدام العبارات المبتذلة المباشرة مثل ("التفاصيل بالداخل"، "اضغط هنا"، "اقرأ المزيد").
- بالنسبة للأخبار الخدمية والرياضية والاقتصادية المحايدة، يُصاغ الملخص كـ "زبدة خبرية مكثفة ومباشرة" دون حاجة للغموض السياسي.
"""
    ...
    # 3. بناء الفرع الديناميكي حسب نوع الخبر
    if is_neutral_cat:
...
- أعد كتابة الخبر كاملاً في حقل (content) دون اختصار أو حذف للأرقام والبيانات والمجموعات.
"""
...
- لا تختصر الوقائع الميدانية ولا تحذف الأرقام في حقل (content)، وركز على الأثر الميداني والاقتصادي والإنساني على المواطن.
...
- إذا تجاوز الفلترة، جرد الخبر من زاويته وتوظيفه السياسي وأعد صياغته كاملاً بأسلوب صحفي رصين دون اختصار في حقل (content).
"""

    # 4. القاعدة التنفيذية النهائية لحقل houthi_iran_exclude وتنسيق JSON
...
يجب أن تكون النتيجة النهائية كائن JSON حصرياً بالتنسيق التالي:
{
  ...
  "excerpt": "الملخص التشويقي الذكي (Smart Hook) بحدود 220 حرفاً",
  "content": "نص الخبر الكامل والموسع أعيد كتابته دون اختصار",
  "houthi_iran_exclude": false
}
"""

    # 5. تجميع البرومبت النهائي الموجه للنموذج
    final_prompt = f"""
{SYSTEM_PROMPT}
