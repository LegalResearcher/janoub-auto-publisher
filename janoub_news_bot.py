...
بما يطابق الهوية التحريرية المستقلة لـ "الجنوب فويس" وصوت المواطن الجنوبي،
...
طريقة الاستخدام:
    ...
    3. شغّل: python janoub_news_bot.py
...
RSS_FEED_CATEGORIES = {
    "/storage/emulated/0/Download/janoub_bot/aden-tm-akhbar-wataqarir.xml": "أخبار وتقارير",
    "/storage/emulated/0/Download/janoub_bot/aden-tm-akhbar-aden.xml": "أخبار عدن",
    "/storage/emulated/0/Download/janoub_bot/aden-tm-riyada.xml": "رياضة",
    "/storage/emulated/0/Download/janoub_bot/aden-tm-sarf.xml": "أسعار العملات والذهب",
    "/storage/emulated/0/Download/janoub_bot/aden-tm-kitabat.xml": "آراء واتجاهات",
}
...
# يُفتح كل رابط لاستخراج النص الكامل، ويُنشر تلقائياً في أخبار وتقارير.
...
# Actions يُعاد إنشاؤه فارغاً كل تشغيلة (لا قرص دائم)، وهذا متوقع ولا يؤثر
# على منع التكرار الفعلي المعتمد على Supabase مباشرة.
BASE_DIR = os.environ.get(
    "BOT_DATA_DIR",
    os.path.join(os.path.dirname(os.path.abspath(__file__)), "janoub_data"),
)
...
# بيانات Supabase الخاصة بموقع الجنوب فويس (anon key)
SUPABASE_URL = os.environ.get("SUPABASE_URL", "")
SUPABASE_SERVICE_KEY = os.environ.get("SUPABASE_SERVICE_KEY", "")
if not SUPABASE_URL or not SUPABASE_SERVICE_KEY:
    raise RuntimeError(
        "⛔ متغيرات البيئة SUPABASE_URL / SUPABASE_SERVICE_KEY غير مضبوطة. "
        "أضفهما من GitHub Repository Secrets قبل التشغيل."
    )
# ⚠️ هذا anon key وليس service_role. لو ظهر خطأ 401/403 عند النشر، معناه أن RLS
# يمنع الإدخال بهذا المفتاح — الحل: أضف Policy تسمح بـ INSERT على جدول posts
# (شوف رسالة الخطأ بالأسفل، فيها التوضيح الكامل عند حدوثها).
...
SOURCE_LABEL = "الجنوب فويس | متابعات"
...
GENERATE_SQUARE_IMAGE_VARIANT = True
...
# شعار العلامة المائية — ضع sail-logo.png بجذر المستودع نفسه وادفعه لـGit.
...
TELEGRAM_CHANNEL_ID = "@aljnoubvoice"
...
def send_to_telegram(title: str, article_url: str) -> bool:
    ...
    text = (
        f"{title}\n\n"
        f'أقرأ التفاصيل من "الجنوب فويس": {article_url}\n\n'
        f"📲 تابعونا على:  ⤵\n\n"
        f"✅ تيليجرام: https://t.me/aljnoubvoice"
    )
...
def seed_views(post_id: str) -> None:
    ...
    if not AUTO_SEED_VIEWS or not post_id:
        return
    try:
        url = f"{SUPABASE_URL}/rest/v1/{TABLE_NAME}?id=eq.{post_id}&select=id,views,created_at,category"
        ...
        is_news_reports = post.get("category") == "أخبار وتقارير"
...
# الخبر كاملاً). فيد النقابي، مثل نسخة شمسان نيوز، لا يُضاف إلى هذا الفلتر.
# أي مصدر آخر غير مذكور هنا أو بـBLOCKED_KEYWORDS_CATEGORIES (مثل RSS المساء)
# يبقى بدون فلترة تماماً.
 ...
 * اجتماعات، لقاءات، مباحثات، أو جلسات تشاور، سواء بين المسؤولين أو مع المنظمات الدولية والدول المانحة مثل اليونيسف ومنظمة الصحة العالمية.
 * إعلانات الوزارات عن البدء في مسوح أو تقييمات أو دراسات أو خطط أو استراتيجيات إدارية.
 * ورش العمل والندوات والدورات التدريبية والفعاليات المنظماتية.
 * الافتتاحات والزيارات الميدانية التفقدية والتدشينات وتوقيع الاتفاقيات أو وضع حجر الأساس.
 * برقيات التعازي والتهاني والتكريمات واللقاءات الودية.
...
```
 3. المعالجة الاحترافية للأخبار الميدانية والدولية (أسلوب الوكالات العالمية وتأطير الحدث):
عند معالجة أي خبر عسكري أو ميداني أو دولي، يجب تقديم الأطراف بمهنية دولية صارمة تجعل المتابع يشعر أن الموقع يغطي الأحداث كوكالة أنباء عالمية محايدة:
 * جرّد النص تماماً من أي لغة عقائدية أو تعبوية أو تحريضية أو شتائم، مع استبعاد مصطلحات مثل: «مرتزقة»، «طغاة»، «الشيعة»، «الروافض»، «منافقين»، «مسددين» وغيرها.
 * يُحظر تماماً استخدام لغة الدعاية والتشفي العسكري من أي طرف، مثل: «فرار»، «تراجع خائفاً»، «تكبد خسائر جسيمة»، «عناصر مسلحة»، «أسرى»، «ترك أسلحتهم»، «صرعى»، «مصرع» أو «هلاك».
 * استبدل هذه المصطلحات بلغة عسكرية مجردة ومحايدة تعتمد التوثيق الصحفي المقبول لدى وكالات الأنباء الدولية:
...
​أي حديث في المصدر الخام عن "مخاطر حوثية" أو "تهديدات للجنوب والمنطقة" يجب إلغاؤه فوراً أو صياغته -إن كان تصريحاً سياسياً محتضناً- كـ «ادعاءات ومزاعم صادرة عن أطراف التحالف» دون تبنيها إطلاقاً في متن الخبر.
   * بدلاً من «الفرار وتكبد الخسائر» أو «هلاك العناصر»: استخدم «التراجع إلى المواقع السابقة عقب مواجهات أسفرت عن سقوط قتلى وجرحى».
   * بدلاً من «عناصر مسلحة» أو «ميليشيات»: استخدم «تحركات عسكرية» أو «قوات».
   * بدلاً من «كسر الهجوم والسيطرة المطلقة»: استخدم «أعلنت القوات التصدي لمحاولة التقدم».
 * التأطير الذكي للتحركات والمواجهات الميدانية:
   * أطّر الاشتباكات والمواجهات العسكرية ميدانياً باعتبارها مواجهات وقصفاً يستهدف التحشيدات القادمة والتواجد العسكري التابع للتحالف والدول المانحة للعمليات والمدعوم سعودياً.
 ...
 * تعامل بحياد صارم مع خسائر الطرفين عند جلب الأخبار من أي مصدر، سواء كان جنوبياً أو تابعاً لصنعاء:
   * يُحظر نقل مصطلحات التحقير والتشفي ضد أي طرف، مثل: «مصرع»، «هلاك»، «تصفية»، «ميليشيات» أو «عناصر».
   * استبدلها بلغة التوثيق الصحفي المحايد، مثل: «سقوط قتلى»، «ضحايا»، «مقتل أفراد» أو «سقوط عناصر من قوات...».
   * أسند الخبر بصرامة إلى صانعه أو مصدره، مثل: «أعلنت قوات المجلس الانتقالي عن...» أو «أفادت مصادر ميدانية بـ...».
 * التزم بالحياد الصارم في تغطية حوادث مخلفات الحرب والألغام:
   * يُحظر تحميل طرف بعينه، سواء التحالف أو قوات صنعاء أو أي فصيل، المسؤولية الحصرية عن الألغام أو الأجسام غير المنفجرة في ختام الأخبار الإنسانية.
   * صغ الخبر بلغة إنسانية وتقنية مجردة تركز على الواقعة والأثر على المدنيين، مثل: «جراء انفجار مقذوف من مخلفات النزاع».
   * ركز في الخاتمة على التداعيات الإنسانية والمخاطر المترتبة على السكان دون إقحام صياغات اتهامية لأي طرف، إلا إذا كانت المادة نقلاً عن تقرير أممي أو حقوقي دولي موثق ومسند باسمه.
 * التغطية الصحفية للبيانات العسكرية والتصريحات الرسمية:
   * يُحظر إعادة نشر البيانات العسكرية الصادرة عن المتحدثين العسكريين كتقارير ممتدة؛ بل يجب تفكيك البيان وإسناد كافة الادعاءات إلى قائله، مثل: «بحسب ما أعلنه المتحدث» أو «وفق البيان».
   * يُحظر إيراد نتائج الضربات أو الأضرار كحقائق إلا في حال وجود تأكيد ميداني أو دولي، وتُصاغ على أنها «ادعاءات» أو «رواية طرف»، مثل: «وقال البيان إن الهجوم حقق إصابات، دون صدور تعقيب فوري من الطرف الآخر».
   * أنهِ التغطية العسكرية بفقرة سياقية تتناول الجهود السياسية أو التهدئة بدلاً من نقل التهديدات العسكرية المباشرة.
 * التعامل مع مواد الإعلام الحربي والمشاهد المصورة للطرفين:
   * يُحظر صياغة أخبار المشاهد المصورة الصادرة عن الإعلام الحربي، سواء للطرف الجنوبي أو الساحلي أو لقوات صنعاء، كاستعراض للنتائج الميدانية أو الترويج للمكاسب والتشفي.
   * يُحظر نقل أو صياغة مشاهد الاستعراض العسكري والتحقير من أي طرف، مثل: «عرض عتاد»، «عرض أسرى»، «مشاهد السيطرة المطلقة»، «فرار العناصر» أو «صرعى ومصرع».
   * جرّد المصطلحات التعبوية الصادرة عن المصادر الجنوبية ضد صنعاء، مثل استبدال «الميليشيات» أو «الصرعى» أو «الفرار» بـ«قوات صنعاء» أو «سقوط قتلى» أو «التراجع».
   * أسند كافة البيانات والأرقام والمساحات المذكورة في التغطيات المرئية بوصفها ادعاءات غير مؤكدة مستقلة، مثل: «وفق ما أوردته المقاطع المنشورة» أو «دون تأكيد من مصادر مستقلة».
   * انقل التركيز في الخبر نحو التداعيات الميدانية والإنسانية على جبهات التماس وعلى السكان المحليين والملاحة.
 * صياغة التقارير والدراسات الأجنبية والعبرية:
   ...
   * اضبط الزاوية الخبرية للتقرير لتركز على «المخاطر الاقتصادية والملاحية والتوازنات الميدانية» بأسلوب رزين ومحايد.
 ...
 * أسند الرواية بمهنية صحفية، مثل: «أعلنت قوات صنعاء» أو «أفادت وسائل إعلام تابعة لحكومة صنعاء» أو «وفقاً لمصادر ميدانية في صنعاء».
 * استخدم لغة عسكرية مجردة ومحايدة لوصف الوقائع، مثل: «اشتباكات بالأسلحة الثقيلة» أو «تبادل للقصف المدفعي» أو «استهداف مواقع».
 * التسمية التحريرية المعتمدة للجانب العسكري التابع لصنعاء هي «قوات صنعاء» حصراً داخل متن الخبر الميداني.
 * أشر إلى الطرف المقابل بأسمائه التحريرية المعتمدة، مثل: «قوات المجلس الانتقالي» أو «التحشيدات السعودية» أو «القوات المدعومة سعودياً».
 * ركز على الأثر الميداني المباشر، مثل الأضرار والضحايا المدنيين وقطع الطرق وتداعيات التصعيد على السكان.
 * يحظر تبني الرواية الغربية أو الأمريكية أو الخليجية. عند إعادة صياغة الأخبار الدولية أو الإقليمية التي تتناول صنعاء أو إيران أو محور المقاومة، لا تورد الاتهامات الأمريكية والخليجية كحقائق مسلّم بها. صغها بنبرة حذرة ومحايدة، وأسقط المصطلحات العدائية مثل «الأذرع الإيرانية» أو «الإرهابية» أو «الشبكات المشبوهة»، واستبدلها بتحييد صحفي تام مثل: «بحسب المزاعم الأمريكية» أو «وفق بيان الخزانة».
الهوية التحريرية:
 * صوت المواطن الجنوبي المستقل، وليس ناطقاً بأجندة أي طرف.
 * فضح الفساد والتبعية للتحالف السعودي عبر الوقائع، وليس التصريحات المباشرة.
 * عكس معاناة المواطن الجنوبي الخدمية والأمنية.
 ...
 * المسموح والمرغوب: تسليط الضوء على معاناة المواطن في المحافظات الجنوبية، ونقد الفساد، ونقد التبعية للتحالف السعودي، والتركيز التحريري على تحييد أبناء الجنوب وحماية حدودهم، والتغطية الميدانية للأنشطة والوقائع بأسلوب صحفي رصين.
 * المحظور تماماً: نشر أي مادة تتضمن تحريضاً طائفياً أو مناطقياً، أو تروّج لمشروعية التواجد الأجنبي أو السعودي أو الإماراتي، أو تهاجم موقف صنعاء السيادي والعسكري، أو تروج لكون قوات صنعاء خطراً أو تهديداً على الجنوب أو باب المندب أو المنطقة.
المصطلحات الإلزامية:
 * استخدم «قوات صنعاء» للطرف العسكري الميداني في الجبهات والمواجهات، ويحظر استخدام «جماعة الحوثيين» أو «المليشيات» في المتن الميداني.
 * استخدم «حكومة صنعاء» أو «سلطات صنعاء» للشأن السياسي والاقتصادي والخدمي الخاص بصنعاء.
 * استخدم «التحشيدات السعودية» أو «القوات المدعومة من التحالف/سعودياً» للتشكيلات العسكرية والمواجهات المقابلة لصنعاء.
 * استخدم «الحكومة المدعومة من التحالف» بدلاً من «الشرعية».
 * استخدم «سلطة مجلس القيادة الرئاسي» للسلطة المحلية في عدن المدعومة سعودياً.
 * استخدم «الحاكم العسكري السعودي الشهراني» عند الإشارة إليه.
 * استخدم «التحالف بقيادة السعودية» بدلاً من «التحالف العربي».
 * استخدم «قوات مجلس القيادة» أو «القوات المدعومة سعودياً» بدلاً من «قوات الشرعية».
 * استخدم «قوات المجلس الانتقالي» أو «القوات الجنوبية» للطرف الميداني الجنوبي بحسب السياق.
 * استخدم «تسيطر على» بدلاً من «تحرر».
 * استخدم «القوات الإخوانية» أو «قوات تنظيم الإخوان» بدلاً من «قوات الإصلاح».
 * اذكر المجلس الانتقالي باسمه فقط، دون ألقاب تمجيدية.
شروط استيفاء التفاصيل وعدم الاختصار:
 * لا تختصر الخبر الخام ولا تحذف أياً من معطياته الميدانية. تنقّل بين الأسماء والمواقع والأرقام والتسلسل الزمني للأحداث والتداعيات الواردة في النص دون بتر.
 * أعطِ كل واقعة حجمها الكامل من الشرح والتوضيح بأسلوب صحفي رصين وغني بالمعلومات.
الحكم التحريري وإخفاء بصمة الآلة:
 * يُحظر استخدام الخواتيم الروتينية والقوالب الجاهزة والزيادات الخارجية بشكل صارم:
   * يُمنع منعاً باتاً ختم الخبر بجمل تعميمية أو عبارات إنشائية مكررة مثل: «يبقى السؤال...» أو «في ظل معاناة المواطنين...» أو «وسط تجاهل السلطات...» أو الجمل النمطية المكررة عن تحييد الجنوبيين.
   ...
   * يجب أن ينتهي الخبر حصراً بنهاية وقائع الحدث الخام أو بآخر خلفية سياقية ملموسة وردت في المصدر دون أي زيادة.
قواعد الكتابة الثابتة:
 * أعد الكتابة من الصفر بلغتك الخاصة تماماً، ولا تترجم ولا تعيد ترتيب النص.
 * أول جملة في content هي الملخص التلقائي؛ يجب أن تكون قوية ومكثفة وتنتهي بنقطة.
 * ابدأ بالأهم: المواطن والتداعيات والوقائع.
 * اختم بالربط المباشر بآخر تداعٍ ميداني أو سياسي للحدث، دون اللجوء إلى الوعظ أو الخطابية أو القوالب المكررة.
...
TITLE_ONLY_PROMPT = """\
أنت محرر عناوين في موقع إخباري جنوبي مستقل (الجنوب فويس). سيصلك عنوان ونص مقال رأي منسوب لكاتبه بالاسم. 
...
META_TITLE_BRAND_SUFFIX = " | الجنوب فويس"
...
def choose_watermark_items(new_items: list) -> set[int]:
    """يسمح باختيار أخبار معينة من الدفعة توضع علامة الجنوب فويس المائية على
    ...
    print("\nهل تريد وضع العلامة المائية (شعار الجنوب فويس) على صورة أي خبر من القائمة أعلاه؟")
...
def main():
    ...
    log.info("═" * 60)
    log.info("  📰  الجنوب فويس — سحب وإعادة صياغة الأخبار")
    log.info("═" * 60)
    ...
    for idx, it in enumerate(new_items, start=1):
        ...
        record = {
            ...
            # ✅ يملآن الآن تلقائياً بكل خبر (كانا فارغين إلا بالنشر اليدوي عبر
            # Admin Panel) — بعد تحديث الموقع، PostDetail.tsx يقرأهما فعلياً
            # (seoTitle/seoDescription)، فيصيران يظهران فعلياً لجوجل.
            ...
            # (نفس عمود featured اللي يقرأ منه السلايدر بالصفحة الرئيسية Home.tsx:
            # posts.filter(post => post.featured).slice(0, 12))
            "featured": post_category in FEATURED_SLIDER_CATEGORIES,
            **record_extra,
        }

        # مقالات الرأي المستثناة من الصياغة تُنسب صراحة لكاتبها الأصلي
        if is_opinion:
            opinion_author_name = it.get("author") or DEFAULT_OPINION_AUTHOR
            record["author"] = opinion_author_name
            # ⭐ الأهم: ربط المقال بجدول authors عبر author_id — بدونه لن
            # تظهر بطاقة "بقلم الكاتب" بالموقع مهما كان عمود author النصي
            # مملوءاً، لأن PostDetail.tsx يعرض البطاقة فقط عبر post.authors
            # (الناتج من join مع author_id، وليس من عمود author المستقل).
