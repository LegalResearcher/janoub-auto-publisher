...
# هذه المكتبة تُستخدم لمطابقة الصور الموجودة صراحةً في blocked_logos فقط.
# لا توجد أي قاعدة حظر مرتبطة بمصدر الخبر أو باسم النطاق.
try:
    import cv2
    import numpy as np
except ImportError:  # pragma: no cover
...
# الحد لا يتحقق إلا بعد مطابقة هندسية مع ملف مرجعي في blocked_logos.
...
def _load_local_blocked_logo_orb_templates() -> list:
    """يجهّز خصائص الصور المرجعية الموجودة في blocked_logos فقط.

    لا تُستخدم أسماء المواقع أو روابط الأخبار في هذا الفحص؛ كل قالب يأتي من
    ملف أضافه المسؤول صراحةً إلى قائمة الحظر المحلية.
    """
...
def _matches_local_blocked_logo_template(raw_bytes: bytes) -> bool:
    """يتحقق من تشابه موضعي مع أي ملف أُضيف إلى blocked_logos.

    فحص RANSAC يمنع التطابقات العشوائية. النتيجة True لا تحجب الخبر: يستدعيها
    مسار الصورة ليترك image_url وthumbnail_image فارغين فقط.
    """
...
def image_contains_blocked_logo(raw_bytes: bytes) -> bool:
    ...
    لا يفحص رابط المصدر ولا اسم الموقع ولا عنوان الخبر. المطابقة تعني منع رفع
    الصورة وحدها؛ يبقى الخبر صالحاً للنشر دون صورة.
    """
